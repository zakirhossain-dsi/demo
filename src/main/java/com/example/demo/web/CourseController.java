...
@Validated
@Tag(name = "Course Controller", description = "This controller exposes interfaces to interact with courses.")
public class CourseController {
    ...
    @PutMapping(value = "/{courseId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<Course> updateCourse(@PathVariable Long courseId, @ModelAttribute @Valid Course course){
        log.info("Got request for updating a course.");

        if(!Objects.equals(courseId, course.getCourseId())){
            throw new IllegalArgumentException("Course id in path variable and request body should be the same.");
        }
